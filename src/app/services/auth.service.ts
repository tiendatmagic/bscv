...
export class AuthService {
  ...
  public getToken = localStorage.getItem('bscv-token');
  public getProfile = localStorage.getItem('profile');
  ...
  constructor(private http: HttpClient, private route: ActivatedRoute, private router: Router, private location: Location, private datePipe: DatePipe) {
  }
  ...
  handleError(error: any) {
    switch (error.status) {
      ...
      case 401:
        ...
        break;
      default:
        if (localStorage.getItem("bscv-token")) {
  ...
  refreshAccessToken() {
    const refreshToken = localStorage.getItem('bscv-renew');
    ...
    this.http.post(`${this.urlEnv}api/auth/refresh`, { refresh_token: refreshToken }).pipe(
      ...
      tap((response: any) => {
        ...
        localStorage.setItem('bscv-token', response.access_token);
        localStorage.setItem('bscv-renew', response.refresh_token);
        this.onLoad = true;
      }),
      catchError((error) => {
        ...
        localStorage.setItem("bscv-token", '');
